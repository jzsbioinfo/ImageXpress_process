...
date_folder = '20190911explain_analysis_use_spark\'
...
Tif_Window.xlim   = [1710 1710+6000];      % [x x+w]
Tif_Window.ylim   = [2255 2255+6000];      % [y y+h]



% Specify the directory to all the microscope images.
Path    = ['Z:\20190814spark\Images\' Name_case_ori '\'];
...
imwrite(uint16(I),[date_folder Name_case '/Cut_BF_' Files(i).name(1:end-5) '.tif'], 'tif' );
imwrite(uint16(BWfinal),[date_folder Name_case '/Cut_Seg_' Files(i).name(1:end-5) '.tif'], 'tif' );
...
Budding = Yeastbow_find_mother(Budding);  % add daughter to the Budding
% show last segmentated figure wilth label

RHX_0_Show_Last_Image_Cells(Budding);
saveas(gcf, fullfile(date_folder,[Name_case_ori 'segmentation.png']))
%% step 3. Deal with merge situation, add merge to budding

for i = 1:length(Budding)
    if i == 1;
        Budding(i).merge = repmat(0, length(Budding(i).mother), 1)';
    else
        % how many cluster comes from same mother
        temp = Budding(i).mother;
        temp = struct("mother", temp);
        for j = 1:length(Budding(i).mother);
            temp.count(j) = histc(temp.mother(:),temp.mother(j));
        end
        
        % 0 labeled cluster do not remove
       for k = 1:length(temp.mother)
            if temp.mother(k) == 0;
                temp.count(k) = 1;
            end
       end
...
% % save the figure
% title([Name_case_ori ' t to ' num2str(i) ' ( time point )']);
% saveas(gcf, fullfile(savedir,'\growth_curve\',[Name_case_ori 'growth_curve.png']))
close all
